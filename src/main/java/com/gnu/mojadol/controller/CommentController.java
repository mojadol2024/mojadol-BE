...
public class CommentController {
    ...
    // 댓글 추가
    @PostMapping("/write")
    public ResponseEntity<?> addComment(//@PathVariable int boardSeq,
                                        @RequestBody CommentRequestDto commentRequestDto,
                                        @RequestHeader("Authorization") String accessToken) {
        // boardSeq 값을 경로에서 바로 받아서 사용
        String userId = jwtUtil.extractUsername(accessToken);
        User user = userRepository.findByUserId(userId);
        commentRequestDto.setUserSeq(user.getUserSeq());
        //commentRequestDto.setBoardSeq(boardSeq); // 경로에서 받아온 값을 설정
